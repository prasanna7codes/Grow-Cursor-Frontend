...
const AMAZON_DELIVERY_CHECK_RUN_FEATURE_ID = 'amazonDeliveryCheck.run';

// The SLA this page ships with: nine days is the slowest delivery still
// considered acceptable, so exactly nine passes and ten is flagged.
const DEFAULT_MAX_DELIVERY_DAYS = 9;
const DEFAULT_TEST_SKU_COUNT = 25;
const MAX_TEST_SKU_COUNT = 500;
...
// one paint. Selection and the end-item calls are unaffected.
...
const FILTER_LABELS = {
  ...
  within_range: 'Within Range',
  flagged_late: 'Flagged (Too Slow)',
  flagged_late_no_orders: 'Flagged (No Orders 90d)',
  flagged_late_with_orders: 'Flagged (Orders 90d)',
  out_of_stock: 'Out of Stock',
  no_delivery_date: 'No Delivery Date',
  no_asin: 'No ASIN',
  became_late: 'Newly Late',
  errors: 'Errors',
...
// Readable English date, e.g. "Jul 19, 2026, 8:22 PM".
...
// The stored delivery date is a plain 'YYYY-MM-DD' calendar day in the
// marketplace's own timezone, so it is formatted from its parts — passing it
// through new Date() would re-interpret it as UTC midnight and shift it a day
...
/**
 * The marketplace-local day the stored day-count was measured from, derived as
 * deliveryDate - deliveryDays.
 *
 * Counts are frozen at scrape time and measured against the MARKETPLACE's
 * calendar, not the viewer's. Those differ for most of the working day in
 * India: at 04:00 IST it is still the previous afternoon in Los Angeles, so a
 * US quote reads one day longer than the viewer's own calendar suggests. That
 * is correct — a US buyer ordering at that moment really does wait that long —
 * but it is baffling without the reference date spelled out.
 */
function getCountReferenceDate(item) {
  if (!item?.deliveryDate || item.deliveryDays == null) return '';
  const match = String(item.deliveryDate).match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) return '';
  const [, year, month, day] = match;
  const ref = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day) - item.deliveryDays));
...
function statusColor(status) {
  ...
  if (status === 'flagged_late' || status === 'error') return 'error';
  if (status === 'no_delivery_date' || status === 'out_of_stock') return 'warning';
  return 'default';
}
...
function getOrderCount90d(sellerItems) {
  return (sellerItems || []).reduce((sum, row) => sum + (row.orderCount90d || 0), 0);
}
...
function getRunScope(run) {
  const size = run.mode === 'test_pilot' && run.skuLimit ? ` | ${run.skuLimit} SKUs` : '';
  return `${run.mode} | ${(run.currencies || []).join(', ') || '-'} | max ${run.maxDeliveryDays}d${size}`;
}

/**
 * The delivery quote as one readable cell: the worst-case date, how many days
 * out that is, and the near end when Amazon gave a range rather than a single
 * day. The day count is what the SLA is judged on, so it leads.
 */
function DeliveryCell({ item }) {
  ...
  const isRange = item.earliestDeliveryDays != null && item.earliestDeliveryDays !== item.deliveryDays;
  const late = item.status === 'flagged_late';
  const referenceDate = getCountReferenceDate(item);
  return (
    <Stack spacing={0.25}>
      <Stack direction="row" spacing={0.75} alignItems="center">
        <Tooltip
          title={referenceDate
            ? `${item.deliveryDays} days counted from ${formatDeliveryDate(referenceDate)} — the marketplace's own date when this SKU was checked, which can be a day behind yours.`
            : ''}
        >
          <Typography variant="body2" sx={{ fontWeight: 900, color: late ? '#b91c1c' : '#047857' }}>
            {item.deliveryDays} {item.deliveryDays === 1 ? 'day' : 'days'}
          </Typography>
        </Tooltip>
        {item.maxDeliveryDays != null && (
          <Typography variant="caption" color="text.secondary">/ max {item.maxDeliveryDays}</Typography>
        )}
      </Stack>
      <Typography variant="caption" color="text.secondary">
        {isRange
          ? `${formatDeliveryDate(item.earliestDeliveryDate)} – ${formatDeliveryDate(item.deliveryDate)}`
      ...
      {rows.map((row) => {
        ...
        return (
          <Paper key={`${row.sellerId}-${row.itemId}`} variant="outlined" sx={{ p: 1.25, mb: 1, borderRadius: 2 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              {row.sku && currentSku && row.sku !== currentSku && (
                <Tooltip title={`Variant listing — exact SKU is ${row.sku}`}>
                  <Chip size="small" variant="outlined" color="info" label={row.sku} sx={{ fontWeight: 800, fontFamily: 'monospace' }} />
                </Tooltip>
              )}
              <Box sx={{ flex: 1 }} />
              ...
              {!endedInfo && (
                <Button size="small" variant="outlined" startIcon={<EditIcon />} disabled={busy} onClick={() => onReviseItem(row)}>
                  Revise
                </Button>
              )}
              {endedInfo ? (
            ...
            {/* The flagged cards are merged into one on the summary row, so the
                order split that used to live there is surfaced here instead. */}
            <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.75 }}>
              <Chip
                ...
                sx={{ fontWeight: 800 }}
              />
              <Chip size="small" variant="outlined" label={`${formatNumber(row.orderCount30d || 0)} / 30d`} sx={{ fontWeight: 800 }} />
              <Chip size="small" variant="outlined" label={`${formatNumber(row.lifetimeOrderCount || 0)} lifetime`} sx={{ fontWeight: 800 }} />
...
export default function AmazonDeliveryDateCheckPage() {
  ...
  const [mode, setMode] = useState('test_pilot');
  const [currencies, setCurrencies] = useState(['USD']);
  const [skuLimit, setSkuLimit] = useState(DEFAULT_TEST_SKU_COUNT);
  const [maxDeliveryDays, setMaxDeliveryDays] = useState(DEFAULT_MAX_DELIVERY_DAYS);
  ...
  const [activeFilters, setActiveFilters] = useState(['flagged_late']);
  ...
  // Map<itemId, rowData> — accumulates selections across every SKU visited in
  // this verify session, so a batch of flagged SKUs can be ended together.
  ...
  // 'manual' keeps the title/price-only revise; 'asin' repoints the listing at
  // a new ASIN using a template, without changing the item ID.
  ...
  const selectedCurrencyList = useMemo(() => {
    if (mode === 'custom' || mode === 'test_pilot') return currencies;
    return ['USD', 'AUD', 'CAD', 'GBP'];
  }, [mode, currencies]);

  // test_pilot and custom both pick a real region; the wider modes cover all.
  const picksRegion = mode === 'custom' || mode === 'test_pilot';
  const selectedCountryValue = picksRegion ? (currencies[0] || 'USD') : 'ALL';

  const fetchRuns = async () => {
    setLoadingRuns(true);
    try {
      const { data } = await api.get('/amazon-delivery-checks/runs', {
        params: { page: runPagination.page, limit: runPagination.limit }
      });
      setRuns(data.runs || []);
  ...
  const fetchRun = async (runId, { showLoading = false } = {}) => {
    ...
    try {
      ...
      const [{ data: runData }, { data: itemsData }] = await Promise.all([
        api.get(`/amazon-delivery-checks/runs/${runId}`, { params: { sellerId } }),
        api.get(`/amazon-delivery-checks/runs/${runId}/items`, {
          params: {
      ...
      setCutoffDate(runData.cutoffDate || '');
  ...
  useEffect(() => {
    ...
    api.get(`/feature-permissions/${AMAZON_DELIVERY_CHECK_RUN_FEATURE_ID}/check`)
  ...
  const openAccessDialog = async () => {
    ...
    try {
      const [{ data: usersData }, { data: permissionData }] = await Promise.all([
        api.get('/users'),
        api.get(`/feature-permissions/${AMAZON_DELIVERY_CHECK_RUN_FEATURE_ID}`)
      ]);
  ...
  const saveAccess = async () => {
    ...
    try {
      const { data } = await api.put(`/feature-permissions/${AMAZON_DELIVERY_CHECK_RUN_FEATURE_ID}`, {
  ...
  const handleEstimate = async () => {
    ...
    try {
      const base = buildRunParams();
      const { data } = await api.get('/amazon-delivery-checks/estimate', {
        params: { ...base, currencies: base.currencies.join(',') }
      });
      setEstimate(data);
    } catch (err) {
  ...
  const handleStart = async () => {
    ...
    setStarting(true);
    try {
      const { data } = await api.post('/amazon-delivery-checks/runs', buildRunParams());
      setActiveRun(data.run);
      setItems([]);
      setItemCounts({});
      setPagination((prev) => ({ ...prev, page: 1, total: 0, totalPages: 1 }));
      setExpandedRows(new Set());
      setActiveFilters(['flagged_late']);
      await fetchRuns();
    } catch (err) {
  ...
  const handleRunAction = async (action) => {
    ...
    setSuccess('');
    try {
      const { data } = await api.post(`/amazon-delivery-checks/runs/${activeRun._id}/${action}`);
  ...
  const handleRecheck = async (item) => {
    setError('');
    setSuccess('');
    setRecheckingId(item._id);
    try {
      const { data } = await api.post(`/amazon-delivery-checks/items/${item._id}/recheck`);
      setSuccess(data.message || `Rechecked ${item.sku}.`);
      // Patch the one row in place so the table doesn't jump: a full refetch
      // would re-sort and could drop the row out of the current filter before
      // the user has read the new result.
      if (data.item) {
        setItems((prev) => prev.map((row) => (row._id === item._id ? { ...row, ...data.item } : row)));
      }
    } catch (err) {
      setError(err.response?.data?.error || err.message || 'Failed to recheck this SKU');
    } finally {
  ...
  const handleMultiFilterToggle = (event) => {
    ...
    if (!enabled) {
      setActiveFilters((prev) => [prev.find((filter) => filter !== 'all') || 'flagged_late']);
    }
    setPagination((prev) => ({ ...prev, page: 1 }));
    setExpandedRows(new Set());
  };
  ...
  // verify drawer scopes down to just that seller's own listing(s) for the SKU.
  ...
  // Open (or navigate) the shared Amazon review window, positioned on the left
  // half of the screen so the verify panel can sit on the right.
  const openAmazonWindow = (url) => {
    ...
    const win = window.open(url, 'amazonDeliveryVerifyWindow', `left=0,top=0,width=${width},height=${height}`);
    if (win) amazonWinRef.current = win;
  };

  /**
   * Listings pre-selected as you step through with the arrow keys.
   *
   * Two cases qualify:
   *  - no_delivery_date — already re-scraped once during the run, so a blank
   *    here means Amazon still would not commit to a date. Selected whatever
   *    the order history says: there is no delivery promise to weigh against.
   *  - flagged_late with nothing sold in 90 days — too slow AND not selling.
   *    If even one seller carrying the SKU has a recent order the whole SKU is
   *    left alone, because a slow item that still sells is a pricing decision
   *    rather than an automatic end.
   *
   * Everything else (within range, out of stock, errors) is never pre-selected.
   */
  const getAutoSelectRows = (data) => {
    if (!data) return [];
    // Scoped to the visible rows, so a seller filter reflects that seller's own
    // order history rather than a different seller's.
    const rows = getVisibleSellerItems(data.sellerItems || []);
    const selectable = rows.filter((row) => !row.endedInfo);
    if (!selectable.length) return [];

    if (data.status === 'no_delivery_date') return selectable;

    if (data.status === 'flagged_late') {
      // Counted across every row including already-ended ones: a listing ended
      // last week still proves the SKU was selling.
      const hasRecentOrder = rows.some((row) => (row.orderCount90d || 0) > 0);
      return hasRecentOrder ? [] : selectable;
    }

    return [];
  };

  // Why a flagged SKU came up with nothing pre-selected — otherwise the empty
  // selection reads as a bug rather than the order guard doing its job.
  const autoSelectSkippedForOrders = verifyData?.status === 'flagged_late'
    && getVisibleSellerItems(verifyData?.sellerItems || []).some((row) => (row.orderCount90d || 0) > 0);
  ...
  const handleVerify = async (item, index) => {
    ...
    try {
      const { data } = await api.get(`/amazon-delivery-checks/items/${item._id}/verify`);
      setVerifyData(data);
      ...
      if (imageItemIds.length && sellerFilter?._id) {
        ...
        api.post('/amazon-delivery-checks/live-images', { sellerId: sellerFilter._id, itemIds: imageItemIds })
          .then(({ data: imageData }) => {
  ...
  // item only needs sku/asin (verifyData or the flat table row both qualify);
  // sellerItem is the specific listing being revised.
  const openReviseDialog = (item, sellerItem) => {
    setReviseTarget({ item, sellerItem });
    setReviseForm({ title: sellerItem.title || '', price: sellerItem.price ?? '' });
  ...
  // content, so the picker never offers one that would fail on submit.
  useEffect(() => {
  ...
  // The revise endpoints are the stock check's — the same eBay revise flow,
  // shared rather than reimplemented, so a listing revised from this page is
  // identical to one revised from there (and lands in the same history).
  const handleGenerateRevisePreview = async () => {
    ...
    try {
      ...
      // that failed validation has to be reported here.
  ...
  // AsinReviewModal hands back an array; this flow only ever has one item.
  const handleApplyAsinRevise = async (listings) => {
    ...
    try {
      ...
      // accepted the revise but the row failed to save, the message is the
      // only signal that the listing is live and needs reconciling.
  ...
  const bulkEndRows = Array.from(selectedRows.values());

  return (
    <Box sx={{ p: 3 }}>
      <PageHeader
        title="Amazon Delivery Date Check"
        subtitle="Check the Amazon delivery date behind every SKU and flag anything quoted slower than your limit."
      />
      ...
      >
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'stretch', md: 'center' }}>
          <Box sx={{ minWidth: 180 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              Narrow the cards and table below to one seller.
            </Typography>
          </Box>
          <Autocomplete
            options={sellers}
          ...
          {sellerFilter && <Button variant="outlined" onClick={() => handleSellerFilterChange(null)}>Clear</Button>}
        </Stack>
      </Paper>
      ...
      {canRun ? (
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, mb: 2 }}>
          <Grid container spacing={2} alignItems="flex-start">
            <Grid item xs={12} md={3}>
              ...
              <Select fullWidth size="small" value={mode} onChange={(event) => setMode(event.target.value)}>
                <MenuItem value="test_pilot">Test pilot — pick region &amp; size</MenuItem>
              ...
              {isTestPilot && (
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
                  Checks exactly this many SKUs that resolve to an ASIN — no-ASIN SKUs are skipped, not counted.
                </Typography>
              )}
            </Grid>
            <Grid item xs={12} md={3}>
              <Typography variant="caption" sx={{ fontWeight: 800, color: 'text.secondary' }}>
                {isTestPilot ? 'Region' : 'Countries'}
              </Typography>
              <Select
                fullWidth
                size="small"
                value={selectedCountryValue}
                disabled={!picksRegion}
                onChange={(event) => setCurrencies([event.target.value])}
              >
                {!picksRegion && <MenuItem value="ALL">All supported countries (USD, AUD, CAD, GBP)</MenuItem>}
            ...
            {isTestPilot && (
              <Grid item xs={6} md={2}>
                <Typography variant="caption" sx={{ fontWeight: 800, color: 'text.secondary' }}>SKUs to test</Typography>
                <TextField
                  fullWidth
                  size="small"
                  type="number"
                  value={skuLimit}
                  onChange={(event) => setSkuLimit(event.target.value)}
                  inputProps={{ min: 1, max: MAX_TEST_SKU_COUNT }}
                  helperText={`1 – ${MAX_TEST_SKU_COUNT}`}
                />
              </Grid>
            )}
            <Grid item xs={6} md={2}>
              <Typography variant="caption" sx={{ fontWeight: 800, color: 'text.secondary' }}>Max Delivery Days</Typography>
              <TextField
                fullWidth
                size="small"
                type="number"
                ...
                helperText={`Flag anything past ${maxDeliveryDays || 0} days`}
              />
            </Grid>
            <Grid item xs={12} md={isTestPilot ? 2 : 4}>
              <Stack direction="row" spacing={1} sx={{ mt: { md: 2 } }} alignItems="center" flexWrap="wrap" useFlexGap>
                <Button
                  variant="outlined"
                ...
                {isRunning && (
                  <Button variant="outlined" color="warning" startIcon={<PauseIcon />} onClick={() => handleRunAction('pause')}>
                    Pause
                  </Button>
                )}
                {isPaused && (
                  <Button variant="outlined" startIcon={<PlayArrowIcon />} onClick={() => handleRunAction('resume')}>
                    Resume
                  </Button>
                )}
                {activeRun && ['queued', 'running', 'paused'].includes(activeRun.status) && (
                  <Button variant="outlined" color="error" startIcon={<StopCircleIcon />} onClick={() => handleRunAction('cancel')}>
                    Cancel
                  </Button>
                )}
                {isSuperAdmin && (
          ...
          {estimate && (
            <Alert severity={estimate.skuLimitUnmet ? 'warning' : 'info'} sx={{ mt: 2 }}>
              {estimate.mode === 'test_pilot' ? (
                <>
                  Test pilot: {formatNumber(estimate.asinFoundCount)} checkable SKU
                  {estimate.asinFoundCount === 1 ? '' : 's'} ready in {estimate.currencies.join(', ')}
                  {' '}(scanned {formatNumber(estimate.candidatesScanned)} base SKUs to find them), estimated
                  {' '}{formatNumber(estimate.creditsEstimated)} credits. Anything quoted later than
                  {' '}{estimate.maxDeliveryDays} days is flagged.
                  {estimate.skuLimitUnmet && (
                    <> Only {formatNumber(estimate.asinFoundCount)} of the {formatNumber(estimate.skuLimit)} requested could be found — the run will check what exists.</>
                  )}
                </>
              ) : (
                <>
                  Estimate: {formatNumber(estimate.totalSkus)} non-empty SKUs selected from the SKU index,
                  {' '}{formatNumber(estimate.asinFoundCount)} base SKUs mapped to ASINs and ready to check,
                  {' '}{formatNumber(estimate.noAsinCount)} with no ASIN found for base SKU, estimated
                  {' '}{formatNumber(estimate.creditsEstimated)} credits. Anything quoted later than
                  {' '}{estimate.maxDeliveryDays} days is flagged.
                </>
              )}
        ...
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, mb: 2 }}>
          <Typography variant="body2" color="text.secondary">
            You have view-only access to this page — you can browse results and verify listings, but starting or configuring new delivery date checks requires permission from a superadmin.
          </Typography>
        </Paper>
      )}
      ...
      {activeRun && (
        <>
          <Alert severity="info" icon={<LocalShippingIcon fontSize="inherit" />} sx={{ mb: 1.5 }}>
            This run flags any SKU whose Amazon delivery date is more than{' '}
            <strong>{activeRun.maxDeliveryDays} days</strong> out
            {cutoffDate ? <> — from today that means arriving after <strong>{formatDeliveryDate(cutoffDate)}</strong></> : null}.
            {' '}A quoted range is judged on its latest date, and the free/standard option is used rather than paid express.
            {activeRun.skuLimitUnmet && (
              <> This test pilot found fewer checkable SKUs than the {formatNumber(activeRun.skuLimit)} requested.</>
            )}
          </Alert>
          ...
          <Grid container spacing={1.5} sx={{ mb: 2 }}>
            ...
            <Grid item xs={6} md={2}><KpiCard label="Within Range" value={itemCounts.within_range || 0} tone="good" active={isFilterActive('within_range')} onClick={() => applyFilter('within_range')} /></Grid>
            {/* One flagged card, not three: the orders / no-orders split now
                lives on the row and in the verify panel, where the actual
                decision about a slow listing gets made. */}
            <Grid item xs={6} md={2}>
              <Tooltip title="Every SKU quoted slower than the limit. Open a row or Verify to see its 90-day order count.">
                <span style={{ display: 'block' }}>
                  <KpiCard label="Flagged (Too Slow)" value={itemCounts.flagged_late || 0} tone="bad" active={isFilterActive('flagged_late')} onClick={() => applyFilter('flagged_late')} />
                </span>
              </Tooltip>
            </Grid>
            <Grid item xs={6} md={2}><KpiCard label="No Delivery Date" value={itemCounts.no_delivery_date || 0} tone="warn" active={isFilterActive('no_delivery_date')} onClick={() => applyFilter('no_delivery_date')} /></Grid>
            <Grid item xs={6} md={2}><KpiCard label="Out of Stock" value={itemCounts.out_of_stock || 0} tone="info" active={isFilterActive('out_of_stock')} onClick={() => applyFilter('out_of_stock')} /></Grid>
            ...
            {runs.map((run) => (
              ...
              >
                <Stack spacing={0.75} sx={{ width: '100%' }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    By {getRunUser(run)} · max {run.maxDeliveryDays}d
                    {run.mode === 'test_pilot' && run.skuLimit ? ` · ${run.skuLimit} SKUs` : ''}
                  </Typography>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip
                      size="small"
                    ...
                    {run.flaggedLateCount > 0 && (
                      <Chip size="small" color="error" label={`${formatNumber(run.flaggedLateCount)} flagged`} sx={{ fontWeight: 800 }} />
                    )}
                  </Stack>
                </Stack>
              </Button>
      ...
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" sx={{ mb: 1 }}>
        ...
        {/* The merged flagged card keeps both halves reachable without
            spending three cards on them. */}
        {isFilterActive('flagged_late') && (
          <>
            <Chip
              size="small"
              variant="outlined"
              color="warning"
              label={`Only with orders (90d): ${formatNumber(itemCounts.flagged_late_with_orders || 0)}`}
              onClick={() => applyFilter('flagged_late_with_orders')}
              sx={{ fontWeight: 800 }}
            />
            <Chip
              size="small"
              variant="outlined"
              label={`Only without orders (90d): ${formatNumber(itemCounts.flagged_late_no_orders || 0)}`}
              onClick={() => applyFilter('flagged_late_no_orders')}
              sx={{ fontWeight: 800 }}
            />
          </>
        )}
      ...
      <Box sx={{ position: 'relative' }}>
        {loadingItems && (
          ...
          >
            <Stack direction="row" spacing={1.25} alignItems="center" sx={{ bgcolor: '#fff', px: 2, py: 1, borderRadius: 2, boxShadow: 2 }}>
              ...
              <Typography variant="body2" sx={{ fontWeight: 800, color: 'text.secondary' }}>Loading items…</Typography>
            </Stack>
          </Box>
        )}
        <TableContainer
        ...
        >
          <Table size="small" sx={{ borderCollapse: 'separate', borderSpacing: '0 4px', backgroundColor: '#f8fafc' }}>
            <TableHead sx={{ background: BRAND_DARK }}>
              <TableRow>
                ...
                <TableCell sx={{ color: '#fff', fontWeight: 900 }}>Verdict</TableCell>
                <TableCell sx={{ color: '#fff', fontWeight: 900 }}>Delivery</TableCell>
                <TableCell sx={{ color: '#fff', fontWeight: 900 }}>Amazon Says</TableCell>
                <TableCell sx={{ color: '#fff', fontWeight: 900 }}>Seller Items</TableCell>
                <TableCell sx={{ color: '#fff', fontWeight: 900 }}>Orders (90d / all)</TableCell>
            ...
            <TableBody>
              {items.map((item, index) => {
                ...
                const orderCount90d = getOrderCount90d(visibleSellerItems);
                ...
                const amazonUrl = getAmazonUrl(item);
                return (
                  <Fragment key={item._id}>
                    <TableRow
                      ...
                      sx={{
                        cursor: 'pointer',
                        backgroundColor: expanded ? expandedPalette.row : '#fff',
                        '& > td': {
                        ...
                        '& > td:first-of-type': {
                          borderLeft: expanded ? `4px solid ${expandedPalette.rail}` : '3px solid transparent',
                    ...
                    >
                      ...
                      <TableCell sx={{ fontFamily: 'monospace' }}>{item.asin || '-'}</TableCell>
                      <TableCell>{item.country}</TableCell>
                      <TableCell>
                        <Chip size="small" color={statusColor(item.status)} label={STATUS_LABELS[item.status] || item.status} sx={{ fontWeight: 800 }} />
                        {item.becameLate && <Chip size="small" color="warning" label="Newly late" sx={{ ml: 1, fontWeight: 800 }} />}
                        {item.status === 'error' && (
                          <Stack spacing={0.25} sx={{ mt: 0.75 }}>
                            <Typography variant="caption" sx={{ fontWeight: 900, color: 'error.main' }}>
                              {item.errorType || 'delivery_check_failed'}{item.errorSource ? ` | ${item.errorSource}` : ''}
                            </Typography>
                      ...
                      <TableCell><DeliveryCell item={item} /></TableCell>
                      <TableCell sx={{ maxWidth: 280 }}>
                        <Typography variant="caption" color="text.secondary" title={item.deliveryText || item.availabilityText || ''}>
                          {item.deliveryText || item.availabilityText || '-'}
                        </Typography>
                      </TableCell>
                      <TableCell>{formatNumber(visibleSellerItems.length)}</TableCell>
                      {/* Replaces the two flagged-by-orders cards: the split is
                          per row, where it is actually actionable. */}
                      <TableCell>
                        <Stack direction="row" spacing={0.5} alignItems="center">
                          <Chip
                            size="small"
                            color={orderCount90d > 0 ? 'warning' : 'default'}
                            label={formatNumber(orderCount90d)}
                            sx={{ fontWeight: 800 }}
                          />
                          <Typography variant="caption" color="text.secondary">/ {formatNumber(orderCount)}</Typography>
                        </Stack>
                      </TableCell>
                      <TableCell align="right">
                        <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                          ...
                          {canRun && (
                            <Tooltip title="Re-scrape this SKU now (costs 1 product credit)">
                              <span>
                                <Button
                                  size="small"
                                  variant="outlined"
                                  disabled={!item.asin || recheckingId === item._id}
                                  startIcon={recheckingId === item._id ? <CircularProgress size={14} /> : <RefreshIcon />}
                                  onClick={(event) => {
                    ...
                    <TableRow>
                      ...
                      >
                        <Collapse in={expanded} timeout="auto" unmountOnExit>
                          <Box sx={{ p: 2, background: expandedPalette.detail }}>
                            {(item.deliveryLines || []).length > 0 && (
                              <Box sx={{ mb: 1.5 }}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 0.5 }}>
                                  Delivery text Amazon returned
                                </Typography>
                                {item.deliveryLines.map((line, lineIndex) => (
                                  <Typography
                                    key={line}
                                    variant="caption"
                                    display="block"
                                    sx={{
                                      ...
                                      // The first line is the one the verdict was read from.
                                      fontWeight: lineIndex === 0 ? 900 : 400,
                                      color: lineIndex === 0 ? BRAND_DARK : 'text.secondary'
                                    }}
                                  >
                                    {lineIndex === 0 ? '▸ ' : '  '}{line}
                                  </Typography>
                                ))}
                                {item.fastestDeliveryDays != null && item.fastestDeliveryDays !== item.earliestDeliveryDays && (
                                  <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
                                    Paid express option would arrive in {item.fastestDeliveryDays} days — not used for the verdict.
                                  </Typography>
                                )}
                                {/* Amazon rotates the buy box, and a different
                                    seller ships at a different speed — the usual
                                    reason this date differs from the one you see. */}
                                {(item.soldBy || item.shipsFrom) && (
                                  <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
                                    Quoted from the offer sold by <strong>{item.soldBy || 'unknown'}</strong>
                                    {item.shipsFrom ? `, ships from ${item.shipsFrom}` : ''}.
                                  </Typography>
                                )}
                                {/* The destination is whatever city the scraper's
                                    proxy exited from — Scrapingdog ignores the
                                    postal code we ask for. Shown because the day
                                    count is only meaningful against it. */}
                                {item.quoteLocation && (
                                  <Typography variant="caption" sx={{ display: 'block', mt: 0.5, color: '#c2410c', fontWeight: 700 }}>
                                    Priced for delivery to {item.quoteLocation}
                                    {item.postalCodeUsed && !item.quoteLocation.includes(item.postalCodeUsed)
                                      ? ` — not the ${item.postalCodeUsed} we requested. Amazon quotes for wherever the scraper connects from, so this date is only comparable with other rows priced for the same place.`
                                      : '.'}
                                  </Typography>
                                )}
                              </Box>
                            ...
                            <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
                              <Typography variant="subtitle2" sx={{ fontWeight: 900 }}>Seller listings</Typography>
                              <Chip size="small" label={item.sku} sx={{ fontWeight: 900 }} />
                              <Chip size="small" label={`${visibleSellerItems.length} item IDs`} />
                              <Chip
                                size="small"
                                color={orderCount90d > 0 ? 'warning' : 'default'}
                                label={`${formatNumber(orderCount90d)} orders in 90d`}
                                sx={{ fontWeight: 800 }}
                              />
                              {sellerFilter && <Chip size="small" variant="outlined" color="primary" label="This seller only" />}
                            </Stack>
                            <Table size="small">
                              <TableHead>
                                <TableRow>
                                  ...
                                  <TableCell>Orders (90d)</TableCell>
                                  <TableCell>Orders (lifetime)</TableCell>
                              ...
                              <TableBody>
                                {visibleSellerItems.map((sellerItem) => (
                                  <TableRow key={`${sellerItem.sellerId}-${sellerItem.itemId}`}>
                                    ...
                                    <TableCell>{sellerItem.price != null ? `${sellerItem.price} ${sellerItem.currency}` : '-'}</TableCell>
                                    <TableCell>
                                      <Chip
                                        size="small"
                                        color={sellerItem.orderCount90d > 0 ? 'warning' : 'default'}
                                        label={formatNumber(sellerItem.orderCount90d)}
                                        sx={{ fontWeight: 800 }}
                                      />
                                    </TableCell>
      ...
      {/* ---------------------------------------------------------------- */}
      {/* Split-screen verify panel: Amazon opens in a window on the left,  */}
      {/* this panel holds the seller listings on the right.               */}
      {/* ---------------------------------------------------------------- */}
      <Drawer
        anchor="right"
        variant="persistent"
        ...
        <Box sx={{ borderBottom: '1px solid #e5e7eb', position: 'sticky', top: 0, background: '#fff', zIndex: 1 }}>
          ...
          <Stack direction="row" spacing={0.75} alignItems="center" flexWrap="wrap" useFlexGap sx={{ px: 1.5, py: 1 }}>
            {verifyData && !verifyLoading && (
              <>
                ...
                <Chip
                  ...
                  sx={{ fontWeight: 800 }}
                />
                {verifyData.deliveryDays != null && (
                  <Chip
                    size="small"
                    color={verifyData.status === 'flagged_late' ? 'error' : 'success'}
                    variant="outlined"
                    label={`${verifyData.deliveryDays}d / max ${verifyData.maxDeliveryDays ?? '-'} · ${formatDeliveryDate(verifyData.deliveryDate)}`}
                    sx={{ fontWeight: 800 }}
                  />
                )}
                {/* The order count that used to be its own KPI card. */}
                <Chip
                  size="small"
                  color={verifyData.hasRecentOrder90d ? 'warning' : 'default'}
                  label={verifyData.hasRecentOrder90d
                    ? `${formatNumber(getOrderCount90d(getVisibleSellerItems(verifyData.sellerItems || [])))} order(s) in 90d`
                    : 'No orders in 90d'}
                  sx={{ fontWeight: 800 }}
                />
              </>
            ...
            {verifyData?.amazonUrl && (
              <Button size="small" variant="outlined" endIcon={<OpenInNewIcon />} onClick={() => openAmazonWindow(verifyData.amazonUrl)}>
                Amazon
              </Button>
            )}
          </Stack>
          {verifyData?.deliveryText && !verifyLoading && (
            <Box sx={{ px: 1.5, pb: 0.75 }}>
              <Typography variant="caption" sx={{ display: 'block', fontFamily: 'monospace', color: 'text.secondary' }}>
                {verifyData.deliveryText}
              </Typography>
              {verifyData.soldBy && (
                <Typography variant="caption" sx={{ display: 'block', color: 'text.secondary' }}>
                  from the offer sold by <strong>{verifyData.soldBy}</strong>
                  {verifyData.shipsFrom ? ` · ships from ${verifyData.shipsFrom}` : ''}
                </Typography>
              )}
              {verifyData.quoteLocation && (
                <Typography variant="caption" sx={{ display: 'block', color: '#c2410c', fontWeight: 700 }}>
                  priced for delivery to {verifyData.quoteLocation}
                </Typography>
              )}
            </Box>
          )}
        ...
        <Box sx={{ p: 2, overflowY: 'auto' }}>
          ...
          {verifyData && !verifyLoading && (
            <Stack spacing={2}>
              ...
              {autoSelectSkippedForOrders && (
                <Alert severity="info" sx={{ py: 0.25, '& .MuiAlert-message': { fontSize: 12 } }}>
                  Too slow, but this SKU has sold in the last 90 days — nothing was pre-selected.
                  Select rows by hand if you still want to end them.
                </Alert>
              )}
              <SellerItemsSection
                ...
                onReviseItem={(row) => openReviseDialog({ sku: verifyData.sku, asin: verifyData.asin, currency: verifyData.currency }, row)}
      ...
      <Dialog open={bulkEndReviewOpen} onClose={() => setBulkEndReviewOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          <Alert severity="warning" sx={{ mb: 1.5 }}>
            Ending a listing on eBay cannot be undone. Check the order counts below before continuing.
          </Alert>
          <Stack spacing={0.75}>
            {bulkEndRows.slice(0, BULK_END_REVIEW_ROW_CAP).map((row) => (
              <Stack key={row.itemId} direction="row" spacing={1} alignItems="center">
                <Chip size="small" label={row.sellerName} sx={{ fontWeight: 800 }} />
                <Typography variant="body2" sx={{ fontFamily: 'monospace', fontWeight: 800 }}>{row.itemId}</Typography>
                <Typography variant="caption" color="text.secondary" noWrap sx={{ flex: 1 }} title={row.title}>
                  {row.sku} — {row.title || '-'}
                </Typography>
                <Chip
                  size="small"
                  color={row.orderCount90d > 0 ? 'warning' : 'default'}
                  label={`${formatNumber(row.orderCount90d || 0)} / 90d`}
                  sx={{ fontWeight: 800 }}
                />
              </Stack>
            ))}
            {bulkEndRows.length > BULK_END_REVIEW_ROW_CAP && (
              <Typography variant="caption" color="text.secondary">
                …and {formatNumber(bulkEndRows.length - BULK_END_REVIEW_ROW_CAP)} more (all of them will be ended).
              </Typography>
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" color="error" onClick={performBulkEnd} startIcon={<CancelIcon />}>
      ...
      <Dialog
        ...
        maxWidth="sm"
      >
        <DialogTitle>{bulkEnding ? 'Ending listings…' : 'Bulk end complete'}</DialogTitle>
        <DialogContent dividers>
          {bulkEndProgress && (
            <>
              <LinearProgress
                variant="determinate"
                value={bulkEndProgress.total ? (bulkEndProgress.done / bulkEndProgress.total) * 100 : 0}
                sx={{ height: 8, borderRadius: 3, mb: 1.5 }}
              />
              <Typography variant="body2" sx={{ fontWeight: 800, mb: 1 }}>
                {formatNumber(bulkEndProgress.done)} of {formatNumber(bulkEndProgress.total)} processed ·
                {' '}{formatNumber(bulkEndProgress.ok)} ended
                {bulkEndFailures.length > 0 ? ` · ${formatNumber(bulkEndFailures.length)} failed` : ''}
              </Typography>
            </>
          )}
          {bulkEndFailures.length > 0 && (
            <Stack spacing={0.5} sx={{ mt: 1 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 900, color: 'error.main' }}>Failures</Typography>
              {bulkEndFailures.map((failure) => (
                <Typography key={failure.itemId} variant="caption" color="text.secondary">
                  <strong>{failure.itemId}</strong> ({failure.sellerName || '-'}, {failure.sku || '-'}): {failure.reason}
                </Typography>
              ))}
            </Stack>
          )}
        ...
        <DialogActions>
          <Button onClick={() => setBulkEndProgressOpen(false)} disabled={bulkEnding}>Close</Button>
        </DialogActions>
      </Dialog>
