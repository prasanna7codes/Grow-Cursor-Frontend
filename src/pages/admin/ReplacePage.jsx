...
export default function ReplacePage() {
  ...
  const fetchReplaceOrders = async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await api.get('/ebay/stored-orders');
      const allOrders = data.orders || [];
      // Filter orders with itemStatus = 'Replace'
      const replaceOrders = allOrders.filter(order => order.itemStatus === 'Replace');
      setOrders(replaceOrders);
    } catch (err) {
      console.error('Failed to fetch replace orders:', err);
  ...
  const handleItemStatusChange = async (orderId, newStatus) => {
    try {
      ...
      // Remove order from list when status changes away from 'Replace'
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            <Typography variant="h5" fontWeight="bold">Replace Orders</Typography>
          </Stack>
          {orders.length > 0 && (
            <Chip
              icon={<SwapHorizIcon />}
              label={`${orders.length} replace order(s)`}
              color="warning"
              variant="outlined"
            />
          )}
      ...
      {orders.length === 0 ? (
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body1" color="text.secondary">
            No orders with Replace status found
          </Typography>
        </Paper>
      ) : (
